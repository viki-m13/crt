#!/usr/bin/env python3
# ============================================================
# Short Strategy Scanner — 10D/30D/60D horizons
#
# Writes:
#   - short/docs/data/full.json                (ranking table + top10 embedded details)
#   - short/docs/data/tickers/{TICKER}.json    (detail payload for every scored ticker)
#
# Scheduling:
#   - Vercel cron runs daily after main scan.
#   - Set FORCE_RUN=1 to bypass gating (manual runs).
#
# Model: CRT SHORT SCANNER — v8-short
#   • Same methodology as main scanner but with 10/30/60 day forward horizons.
...
# Horizons
HORIZONS_DAYS = {"10D": 10, "30D": 30, "60D": 60}
HORIZON_WEIGHTS = {"10D": 0.50, "30D": 0.35, "60D": 0.15}
...
# Outputs
OUT_DIR = os.path.join("short", "docs", "data")
...
# Analogs -> horizon summaries + units
    ...
    row = {
        ...
        "similar_cases": int(n_eff),
        "typical": {
            "10D": h_summaries.get("10D", {}).get("median", None),
            "30D": h_summaries.get("30D", {}).get("median", None),
            "60D": h_summaries.get("60D", {}).get("median", None),
        },
...
def main():
    ...
    res = res.sort_values(["prob_60d", "conviction"], ascending=[False, False]).reset_index(drop=True)
    ...
    mark_ran_today()
    print(f"[OK] Wrote {len(rows)} tickers -> short/docs/data (as_of={as_of})")
