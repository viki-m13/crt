"""
Vercel serverless cron — Main CRT Rebound Scanner.

Runs the daily_scan.py script, then pushes docs/data/ files to GitHub.
Schedule: 22:00 UTC Mon-Fri (5pm ET)
"""
...
def run_scan():
    """Run the main daily scan and push results to GitHub."""
    # Add scripts dir to path so daily_scan can be imported
    ...
    # Change to repo root so relative paths in daily_scan.py work
    ...
    def do_GET(self):
        try:
            ...
            body = json.dumps({
                "ok": True,
                "pushed": len(pushed),
                "files": pushed[:20],  # cap list size in response
            })
