#!/usr/bin/env python3
"""
TMD-ARC Full Pipeline Runner
==============================
Runs the complete experiment pipeline:
1. Download data
2. Compute features
3. Run autonomous experiment loop (AgentHub-inspired)
...
from src.data_pipeline import load_or_download, split_data
...
def main():
    print("=" * 70)
    print("TMD-ARC: Temporal Momentum Dispersion with Adaptive Regime Cascade")
    ...
    baseline_result = engine.run("2010-01-01", "2019-12-31", verbose=True)
    ...
    # Get best config from experiments
    ...
    if best_cfg_dict:
        ...
        print("\nNo improvement found, using baseline config")
        best_config = baseline_config

    # === STEP 4: VALIDATION ===
    print("\n[STEP 4/5] Running validation suite...")

    # First, run best config on validation period
    ...
    print("-" * 57)
    bm = baseline_metrics
    print(f"{'Train (2010-2019)':<25} {bm['sharpe']:>8.3f} "
