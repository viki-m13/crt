"""Build v8 webapp data.json for the PIT-S&P-500 strategy.

V8 is an HONEST Pareto-improvement over the deployed v3 strategy:

  - K_normal=3, K_recovery=3, K_bull=2  (regime-conditional concentration)
  - Inverse-vol weighting of the basket within each rebalance.
  - Same regime gate, same scorer, same WF model, same universe.

Result on PIT S&P 500 home universe (10 walk-forward splits 2003-2025):

| Metric           | v3 deployed | V8 winner | Δ |
|------------------|------------:|----------:|---:|
| Full CAGR        | 39.77%  | 41.48%  | +1.71pp |
| WF mean CAGR     | 42.80%  | 46.08%  | +3.28pp |
| WF min CAGR      | 14.49%  | 24.06%  | +9.57pp |
| Sharpe           | 0.955   | 1.020   | +0.065  |
| MaxDD            | -49.83% | -46.40% | +3.43pp |
| WF n positive    | 10/10   | 10/10   | tied    |
| WF n beats SPY   | 9/10    | 10/10   | +1      |

Schema is backward-compatible with `docs/monthly_dca.js`.

Strategy spec: ml_3plus6 | kn=3/kr=3/kb=2 | invvol | tight gate | h=6m | cost=10bp
Universe: PIT S&P 500 members at each rebalance month-end.

Run from repo root:
    python3 -m experiments.monthly_dca.v8.build_webapp_v8
"""
...
STRATEGY_SPEC = {
    ...
    "K_recovery": 3,
    "K_bull": 2,
    "weighting": "inverse-volatility (1/vol_1y, normalised within basket)",
    "regime_gate": "tight",
    "regime_gate_rule": (
    ...
    "universe": "PIT S&P 500 members at each rebalance month-end",
    "rebalance_rule": (
        "Hold each basket for 6 months. Reform basket on month T if "
        "(T - last_rebalance) >= 6m or regime transitions to/from cash. "
        "In bull regime (SPY 12m >= 10% AND above 200dma) hold the top 2 "
        "highest-conviction picks; in normal/recovery regime hold the top 3."
    ),
}

WINNER_NAME = "v8_pit_sp500_ml_3plus6_kn3_kr3_kb2_invvol_h6"
...
def run_full_sim(
    ...
    vol_by_asof: dict[pd.Timestamp, pd.Series] | None = None,
    cost_bps: float = 10.0,
    hold_months: int = 6,
    k_normal: int = 3,
    k_recovery: int = 3,
    ...
    """Run the v8 strategy honestly across the full window.
    ...
    for i, m in enumerate(months):
        ...
        if do_reb:
            ...
            if regime == "crash":
                ...
                # K depends on regime
                K = {"recovery": k_recovery, "bull": k_bull,
                     "normal": k_normal}.get(regime, k_normal)
                ...
                if len(sub_pit) == 0:
                    ...
                    if len(top) < K:
                        ...
                        # Inverse-volatility weighting (V6 robustness)
                        if weighting == "invvol" and vol_by_asof is not None:
                            vol = vol_by_asof.get(m)
                            vv = np.array([
                                float(vol[t]) if (vol is not None and t in vol.index
                                                  and not pd.isna(vol[t]) and float(vol[t]) > 0)
                                else 0.40
                                for t in cur_picks
                            ])
                            invv = 1.0 / vv
...
def main():
    ...
    # Pre-load vol_1y per asof for inverse-volatility weighting
    print("  Loading vol_1y per asof for inverse-vol weighting...")
    vol_by_asof: dict = {}
    for asof, fp in feature_by_asof.items():
        try:
            df = pd.read_parquet(fp, columns=["vol_1y"])
            vol_by_asof[asof] = df["vol_1y"]
        except Exception:
            continue

    print("\n=== Running v8 simulation over live window ===")
    rets_log, trade_log, live_state = run_full_sim(
        members_g, preds_wf, preds_live, spy_features, monthly_returns,
        monthly_prices, vol_by_asof=vol_by_asof,
        cost_bps=10.0, hold_months=6,
    ...
    wf_split_rows = []
    wf_aggregate_rows = []
    wf_path = PIT / "v8_walkforward.csv"
    if wf_path.exists():
    ...
    # === Bias overlay ===
    bias_rows = []
    bias_path = PIT / "v8_bias_sensitivity.csv"
    if bias_path.exists():
    ...
    # === Sub-period CAGR ===
    sub_period_rows = []
    sp_path = PIT / "v8_sub_periods.csv"
    if sp_path.exists():
    ...
    gen_rows = []
    gen_path = PIT / "v8_generalize.csv"
    if gen_path.exists():
    ...
    # === Sensitivity ===
    sens_rows = []
    sens_path = PIT / "v8_winner_sensitivity.csv"
    if sens_path.exists():
    ...
    most_picked = []
    mp_path = PIT / "v8_most_picked.csv"
    if mp_path.exists():
    ...
    data = {
        ...
        "recommended_strategy": {
            ...
            "description": (
                "v8 PIT S&P 500 strategy. Walk-forward Gradient Boosted Trees "
                "ranking model (3m+6m forward-rank ensemble) restricted to "
                "point-in-time S&P 500 constituents at each rebalance. "
                "Top-3 picks in normal/recovery regimes, top-2 in bull "
                "regime, weighted by inverse volatility. Held 6 months; "
                "'tight' regime gate goes 100% cash on SPY 21d <= -8% or "
                "6m <= -5%. Walk-forward 46.08% MEAN OOS CAGR over 10 splits "
                "2003-2025 with 10/10 positive AND 10/10 beating SPY. "
                "Pareto-improves the prior v3 strategy on CAGR (+3.3pp), "
                "Sharpe (+0.07), and MaxDD (+3.4pp)."
            ),
        },
        "growth": growth,
        "year_by_year": {
