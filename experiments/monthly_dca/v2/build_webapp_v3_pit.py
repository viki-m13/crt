"""Build v3 webapp data.json for the PIT-S&P-500 strategy.

Schema is backward-compatible with `docs/monthly_dca.js` so the existing
front-end continues to render.  New fields:

  - strategy_version: "v3-pit-sp500"
  - strategy_spec: { scorer, K, weighting, regime_gate, hold_months, cost_bps, universe }
  - universe_membership_n: per-month asof -> count
  - last_rebalance_date, next_rebalance_date, months_since_rebalance
  - sub_periods, multi_universe_generalisation, parameter_sensitivity
  - bias_sensitivity in v3 format

Strategy: ml_3plus6 | K=3/3/3 | EW | tight gate | hold=6m | cost=10bp
...
WINNER_NAME = "v3_pit_sp500_ml_3plus6_k3_h6"
...
def main():
    ...
    wf_split_rows = []
    wf_aggregate_rows = []
    wf_path = PIT / "v3_ml_3plus6_walkforward.csv"
    if wf_path.exists():
    ...
    # === Bias overlay ===
    bias_rows = []
    bias_path = PIT / "v3_ml_3plus6_bias_sensitivity.csv"
    if bias_path.exists():
    ...
    # === Sub-period CAGR ===
    sub_period_rows = []
    sp_path = PIT / "v3_ml_3plus6_sub_periods.csv"
    if sp_path.exists():
    ...
    gen_rows = []
    gen_path = PIT / "v3_generalize.csv"
    if gen_path.exists():
    ...
    # === Sensitivity ===
    sens_rows = []
    sens_path = PIT / "v3_winner_sensitivity.csv"
    if sens_path.exists():
    ...
    most_picked = []
    mp_path = PIT / "v3_ml_3plus6_most_picked.csv"
    if mp_path.exists():
    ...
    # === Drawdown ledger ===
    drawdowns = []
    dd_path = PIT / "v3_ml_3plus6_drawdowns.csv"
    if dd_path.exists():
    ...
    data = {
        ...
        "recommended_strategy": {
            ...
            "description": (
                "v3 PIT S&P 500 strategy.  Walk-forward Gradient Boosted Trees "
                "ranking model (3m+6m forward-rank ensemble) restricted to "
                "point-in-time S&P 500 constituents at each rebalance.  "
                "Top-3 picks held for 6 months; equal-weight; 'tight' regime "
                "gate goes 100% cash on SPY 21d <= -8% or 6m <= -5%. "
                "Walk-forward 42.80% MEAN OOS CAGR over 10 splits 2003-2024 "
                "with 10/10 positive, 9/10 beating SPY. 1.5x annual turnover."
            ),
        },
        "growth": growth,
        "year_by_year": {
