#!/usr/bin/env python3
# ============================================================
# Max Strategy Scanner — 10D/30D/60D/3M/6M/1Y/3Y/5Y horizons
#
# Writes:
#   - max/docs/data/full.json                (ranking table + top10 embedded details)
#   - max/docs/data/tickers/{TICKER}.json    (detail payload for every scored ticker)
#
# Scheduling:
#   - GitHub Actions runs daily after market close.
#   - Set FORCE_RUN=1 to bypass gating (manual runs).
#
# Model: CRT MAX SCANNER — v9-max
#   • Full-horizon analog matcher: 10/30/60 trading days + 3/6/12/36/60 months.
#   • Washout Meter (0–100): how washed-out/sold-off it looks.
#   • Edge Score    (0–100): rebound edge from historical analogs.
#   • Final Score   (0–100): Edge Score amplified by Washout.
#   • Covers both stocks (curated 100) and crypto (100+).
# ============================================================
...
MIN_HISTORY_BARS = LB_LT + BETA_LB + LB_ST + 60

# Analogs
ANALOG_K = 250
ANALOG_MIN = 80
ANALOG_MIN_SEP_DAYS = 5
...
# Horizons — short-term (trading days) through long-term (~5 years of trading days)
HORIZONS_DAYS = {
    "10D": 10,
    "30D": 30,
    "60D": 60,
...
# Scoring blends short-term signal strongest (that's the rebound thesis) but still
# surfaces a long-horizon component — the UI decides which horizon to present.
HORIZON_WEIGHTS = {
    "10D": 0.25,
    "30D": 0.20,
    "60D": 0.15,
...
def horizon_unit(confirm: float, stats: dict, n_target: int) -> float:
    ...
    med_term = sigmoid(med / 0.05)
    tail_pen = sigmoid(max(0.0, -p10) / 0.05)
...
def _compute_10d_winrate_at(feat: pd.DataFrame, X: pd.DataFrame, regimes: pd.Series,
                           now_idx: pd.Timestamp, eligible=None) -> float:
    """10-day win probability from analog matching at a specific historical point."""
    if now_idx not in X.index or (not X.loc[now_idx].notna().all()):
        return np.nan
    y = feat.get("fwd_10D", None)
    if y is None:
        return np.nan
...
def compute_conviction_series(
    ...
    """Historical Opportunity Score (quality × 10D prob × pullback gate) for chart shading.

    Uses today's quality (stable over chart window), the 10D win rate
    ...
    vals = {}
    for t in sample_idx:
        win_1y = _compute_10d_winrate_at(feat, X, regimes, t, eligible=eligible)
...
# Analogs -> horizon summaries + units
    ...
    for h, _w in HORIZON_WEIGHTS.items():
        ...
        if s["n"] >= ANALOG_MIN:
            ...
            h_n.append(s["n"])
            if analog_regimes_for_conf is None and h == "10D":
    ...
    row = {
        ...
        "risk": risk,
        "similar_cases": int(n_eff),
        "typical": {h: h_summaries.get(h, {}).get("median", None) for h in HORIZONS_DAYS.keys()},
        # Per-horizon win probability (0-100), median, and downside (p10)
        "prob_10d": float(h_summaries.get("10D", {}).get("win", 0) * 100) if "10D" in h_summaries else None,
        "prob_30d": float(h_summaries.get("30D", {}).get("win", 0) * 100) if "30D" in h_summaries else None,
        "prob_60d": float(h_summaries.get("60D", {}).get("win", 0) * 100) if "60D" in h_summaries else None,
        "prob_3m":  float(h_summaries.get("3M",  {}).get("win", 0) * 100) if "3M"  in h_summaries else None,
        "prob_6m":  float(h_summaries.get("6M",  {}).get("win", 0) * 100) if "6M"  in h_summaries else None,
        ...
        "median_3m":  h_summaries.get("3M",  {}).get("median", None),
        "median_6m":  h_summaries.get("6M",  {}).get("median", None),
...
def main():
    ...
    # Universe — curated stocks + full crypto universe
    ...
    print(f"[UNIVERSE] stocks={len(tickers)} | crypto={len(CRYPTO_UNIVERSE)} | extra={len(extra_tickers)} | universe={len(universe)}")
    ...
    # Rank by the best-available horizon probability per ticker, then final_score.
    # UI will let the user pick a specific horizon.
    prob_cols = ["prob_10d", "prob_30d", "prob_60d", "prob_3m", "prob_6m", "prob_1y", "prob_3y", "prob_5y"]
    for c in prob_cols:
        if c not in res.columns:
            res[c] = None
    res["best_prob"] = res[prob_cols].max(axis=1, skipna=True)
    res = res.sort_values(["best_prob", "final_score"], ascending=[False, False]).reset_index(drop=True)
